...
def main():
    ...
    # read contexts
    ctx_df = pd.read_csv(options.ctx_tsv, sep='\t', encoding='utf-8')[['chrom', 'start', 'end']].copy().reset_index(drop=True)
    ...
    for si, [_, seq_row] in tqdm(enumerate(seq_df.iterrows()), total=len(seq_df)):
        ...
        for ci, [_, ctx_row] in enumerate(ctx_df.iterrows()):
            ...
            ref_1hot = dna.dna_1hot_mask_species_encoding(ctx_ref, num_species=num_species, species_index=params_train['r64_idx'])
