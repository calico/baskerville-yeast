...
def main():
    ...
    # Group targets by description
    def get_group(file_path):
        if '/group/idea/basenji_yeast/data/rossi_et_al/bigwigs/rossi' in file_path:
            return 'CHiP-exo'
        elif '/group/yeast_ml/data/bigwigs/histone_mark' in file_path:
            return 'Histone Marks'
        elif '/group/yeast_ml/data/bigwigs/ID2/' in file_path:
            return 'RNA-seq'
        else:
            return 'Other'

    targets_strand_df['group'] = targets_strand_df['file'].apply(get_group)
    print("Targets strand:", targets_strand_df.shape)
    print(targets_strand_df)
    ...
    # Do the normalization for each group separately
    for group in targets_strand_df['group'].unique():
        group_mask = targets_strand_df['group'] == group
        group_indices = np.where(group_mask)[0]
        
        # Quantile normalize
        gene_targets_norm[:, group_indices] = quantile_normalize(gene_targets[:, group_indices], ncpus=2)
        gene_preds_norm[:, group_indices] = quantile_normalize(gene_preds[:, group_indices], ncpus=2)
        
        # Mean normalize
        gene_targets_norm[:, group_indices] -= gene_targets_norm[:, group_indices].mean(axis=-1, keepdims=True)
        gene_preds_norm[:, group_indices] -= gene_preds_norm[:, group_indices].mean(axis=-1, keepdims=True)


    # Do the normalization all together
    ...
    # Update accuracy stats calculation
    acc_pearsonr = []
    acc_r2 = []
    acc_npearsonr = []
    acc_nr2 = []
    ...
    for ti in range(num_targets_strand):
        ...
        wr_ti = gene_within[var_mask, ti].mean()
    ...
    # Print overall results and results by group
    ...
    for group in acc_df['group'].unique():
        ...
        print(f'  Normalized PearsonR:  {group_df.pearsonr_norm.mean():.4f}')
        print(f'  Normalized R2:        {group_df.r2_norm.mean():.4f}')
        print(f'  Within-gene PearsonR: {group_df.pearsonr_gene.mean():.4f}')
