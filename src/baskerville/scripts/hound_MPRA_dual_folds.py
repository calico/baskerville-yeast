...
def main():
    ...
    # sed
    sed_options = OptionGroup(parser, 'borzoi_sed_replace_folds.py options')
    ...
    fold_options.add_option(
        '--tsv',
        dest='tsv_file',
        default='/home/jlinder/seqnn/data/enhancers/enhancers_k562.tsv'
    )
    ...
    for ci in range(options.crosses):
        for fi in fold_index:
            ...
            cmd_fold = '%s time python /home/kchao10/scr4_ssalzbe1/khchao/Yeast_ML/baskerville-yeast/src/baskerville/scripts/hound_MPRA_dual.py %s %s' % (cmd_base, params_file, model_file)

            # variant scoring job
            job_out_dir = it_out_dir
            if not options.restart or not os.path.isfile('%s/sed.h5'%job_out_dir):
                cmd_job = '%s %s' % (cmd_fold, options.tsv_file)
                ...
                #         mem=60000, time='48:0:0')

                j = slurm.Job(cmd_job, '%s' % name,
                        '%s.out'%job_out_dir, '%s.err'%job_out_dir, '%s.sb'%job_out_dir,
                        queue=options.queue, gpu=0, time='48:0:0')
                jobs.append(j)
