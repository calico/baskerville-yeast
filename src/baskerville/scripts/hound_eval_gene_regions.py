...
def main():
    ...
    ###############################################################################
    # 9) GENE-LEVEL ACCURACY STATS (averaging across tracks)
    ###############################################################################
    # For each gene, we measure correlation across tracks (1 value per track).
    # We'll also keep the "within-gene" correlation but in a gene-centric form.

    acc_gene_pearsonr         = []
    acc_gene_r2               = []
    acc_gene_pearsonr_norm    = []
    acc_gene_r2_norm          = []
    acc_gene_wpearsonr        = []
    acc_gene_wpearsonr_norm   = []
    coverage                  = []
    coverage_norm             = []

    for i, gene_id in enumerate(gene_ids):
        # -- correlation across tracks (unnormalized)
        r_i = pearsonr(gene_targets[i, :], gene_preds[i, :])[0]
        r2_i = explained_variance_score(gene_targets[i, :], gene_preds[i, :])

        # -- correlation across tracks (gene-level norm)
        nr_i  = pearsonr(gene_targets_norm[i, :], gene_preds_norm[i, :])[0]
        nr2_i = explained_variance_score(gene_targets_norm[i, :],
                                        gene_preds_norm[i, :])

        acc_gene_pearsonr.append(r_i)
        acc_gene_r2.append(r2_i)
        acc_gene_pearsonr_norm.append(nr_i)
        acc_gene_r2_norm.append(nr2_i)

        # coverage
        coverage_mean = gene_targets[i, :].sum()
        gene_len = gene_lengths[gene_id]
        coverage_norm_by_length = coverage_mean / gene_len
        coverage.append(coverage_mean)
        coverage_norm.append(coverage_norm_by_length)

        # -- within-gene correlation (unnormalized),
        #    but only for tracks whose bin-level variance > 80th percentile
        w_within_values = []
        w_within_values_norm = []

        for ti in range(num_targets_strand):
            if gene_wvar[i, ti] > wvar_t[ti]:
                w_within_values.append(gene_within[i, ti])
            if gene_wvar_norm[i, ti] > wvar_t_norm[ti]:
                w_within_values_norm.append(gene_within_norm[i, ti])

        if len(w_within_values) > 0:
            gene_wpearson = np.nanmean(w_within_values)
        else:
            gene_wpearson = np.nan

        if len(w_within_values_norm) > 0:
            gene_wpearson_norm = np.nanmean(w_within_values_norm)
        else:
            gene_wpearson_norm = np.nan

        acc_gene_wpearsonr.append(gene_wpearson)
        acc_gene_wpearsonr_norm.append(gene_wpearson_norm)

    acc_genes_df = pd.DataFrame({
        'gene_id': gene_ids,
        'pearsonr': acc_gene_pearsonr,
        'r2': acc_gene_r2,
        'pearsonr_norm': acc_gene_pearsonr_norm,
        'r2_norm': acc_gene_r2_norm,
        'pearsonr_gene': acc_gene_wpearsonr,         # within-gene unnorm
        'pearsonr_gene_norm': acc_gene_wpearsonr_norm, # within-gene norm
        'coverage': coverage,
        'coverage_norm': coverage_norm
    })
    acc_genes_df.to_csv(f"{options.out_dir}/gene_acc.txt", sep='\t', index=False)

    print("\nGene-level overall results (averaging across genes):")
    print("  PearsonR (unnorm):           %.4f" % np.mean(acc_genes_df.pearsonr))
    print("  R2 (unnorm):                 %.4f" % np.mean(acc_genes_df.r2))
    print("  PearsonR (gene-level norm):  %.4f" % np.mean(acc_genes_df.pearsonr_norm))
    print("  R2 (gene-level norm):        %.4f" % np.mean(acc_genes_df.r2_norm))
    print("  Within-gene PearsonR (unnorm, 80th%%):  %.4f" % np.nanmean(acc_genes_df.pearsonr_gene))
    print("  Within-gene PearsonR (norm, 80th%%):    %.4f" % np.nanmean(acc_genes_df.pearsonr_gene_norm))
    print("  Coverage (norm by length):   %.4f" % np.nanmean(acc_genes_df.coverage_norm))
    print("  Coverage (raw):              %.4f" % np.nanmean(acc_genes_df.coverage))
