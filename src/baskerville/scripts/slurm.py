...
class Job:
    ...
    def launch(self):
        ...
        if self.queue == "a100" or self.queue == "ica100":
            print('#SBATCH -A ssalzbe1_gpu', file=sbatch_out)
        elif self.queue == "bigmem":
            print('#SBATCH -A ssalzbe1_bigmem', file=sbatch_out)
        print(self.cmd, file=sbatch_out)
        sbatch_out.close()
        print('sbatch %s' % sbatch_file)
